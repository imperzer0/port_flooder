...
//insert new proxy in front of the list
	...
	proxy->proxyinfolist->
			proxytype = proxytype;
	proxy->proxyinfolist->
			proxyhost = (proxyhost ? strdup(proxyhost) : nullptr);
	proxy->proxyinfolist->
			proxyport = proxyport;
	proxy->proxyinfolist->
			proxyuser = (proxyuser ? strdup(proxyuser) : nullptr);
	proxy->proxyinfolist->
			proxypass = (proxypass ? strdup(proxypass) : nullptr);
	proxy->proxyinfolist->
			next = next;
	return 0;
}
...
//send connect command
		...
		else
		{
			...
			*(uint16_t*)(((uint8_t*)&(request->dst_addr)) + 1 + dsthostlen) =
			...
			write_log_info(
					proxy, PROXYSOCKET_LOG_INFO,
					"Connecting to destination host: %s:%lu", dsthost, (unsigned long)dstport
			);
			if (send(sock, (void*)request, requestlen, 0) < requestlen)
...
//receive response
		...
		switch (initrecv[1])
		{
			case SOCKS5_STATUS_SUCCESS :
				write_log_info(
						proxy, PROXYSOCKET_LOG_INFO,
						"SOCKS5 proxy connection established to: %s:%lu", inet_ntoa(*(struct in_addr*)&hostaddr), (unsigned long)dstport
				);
				break;
		...
		if (initrecv[0] != 0)
			write_log_info(
					proxy, PROXYSOCKET_LOG_WARNING,
					"Expected SOCKS5 response reserved value to be zero (%u)", (unsigned int)initrecv[0]
			);
		switch (initrecv[1])
		{
			case 1 :
			{
				...
				write_log_info(
						proxy, PROXYSOCKET_LOG_INFO,
						"SOCKS5 connection bound to IPv4 address: %s", inet_ntoa(*(struct in_addr*)&bindaddr));
			}
				break;
			case 3 :
			{
				...
				ERROR_DISCONNECT_AND_ABORT("Connection lost while reading connect response from SOCKS5 proxy")
				if ((bindaddr = (char*)malloc(bindaddrlen + 1)) == nullptr)
				ERROR_DISCONNECT_AND_ABORT(memory_allocation_error)
				...
				write_log_info(
						proxy, PROXYSOCKET_LOG_INFO,
						"SOCKS5 connection bound to host: %s", bindaddr
				);
				free(bindaddr);
			}
				break;
...
//write_log_info(proxy, PROXYSOCKET_LOG_INFO, "SOCKS5 connection bound to IPv6 address: %s", inet_ntoa(*(struct in_addr*)&bindaddr));
		...
		write_log_info(
				proxy, PROXYSOCKET_LOG_INFO,
				"SOCKS5 connection bound to port: %lu", (unsigned long)bindport
		);
	}
...
//connect to the HTTP proxy server
		write_log_info(
				proxy, PROXYSOCKET_LOG_INFO,
				"Preparing to connect to web proxy: %s:%lu", proxyinfo->proxyhost, (unsigned long)proxyinfo->proxyport
		);
		if ((sock = proxyinfo_connect(proxy, proxyinfo->next, proxyinfo->proxyhost, proxyinfo->proxyport, errmsg)) == INVALID_SOCKET)
//ERROR_DISCONNECT_AND_ABORT("Error connecting to web proxy: %s:%lu", proxyinfo->proxyhost, (unsigned long)proxyinfo->proxyport);/////
		...
		write_log_info(
				proxy, PROXYSOCKET_LOG_INFO,
				"Connected to web proxy: %s:%lu", proxyinfo->proxyhost, (unsigned long)proxyinfo->proxyport
		);
//prepare basic authentication data
		char* proxyauth = nullptr;
		if (proxyinfo->proxyuser && *proxyinfo->proxyuser)
		{
			write_log_info(
					proxy, PROXYSOCKET_LOG_INFO,
					"Proxy authentication user: %s", proxyinfo->proxyuser
			);
			char* userpass;
			int proxyuserlen = strlen(proxyinfo->proxyuser);
			if ((userpass = (char*)malloc(proxyuserlen + (proxyinfo->proxypass ? strlen(proxyinfo->proxypass) : 0) + 2)) == nullptr)
			ERROR_DISCONNECT_AND_ABORT(memory_allocation_error)
...
//connect proxy to destination
		...
		write_log_info(
				proxy, PROXYSOCKET_LOG_INFO,
				"Sending HTTP proxy CONNECT %s:%u", host, dstport
		);
		if ((proxycmd = (char*)malloc(proxycmdlen)) == nullptr)
		ERROR_DISCONNECT_AND_ABORT(memory_allocation_error)
		snprintf(
				proxycmd, proxycmdlen,
				"CONNECT %s:%u HTTP/1.0%s%s\r\n\r\n", host, dstport, (proxyauth ? "\r\nProxy-Authorization: Basic " : ""),
				(proxyauth ? proxyauth : ""));
		if (proxyauth)
			free(proxyauth);
		...
		if (result != 200)
			write_log_info(
					proxy, PROXYSOCKET_LOG_DEBUG,
					"HTTP proxy response code %i, details:\n%s", result, response
			);
		free(response);
		response = nullptr;
		...
		write_log_info(
				proxy, PROXYSOCKET_LOG_INFO,
				"Web proxy connection established to: %s:%lu", host, (unsigned long)dstport
		);
...
//use direct connection if proxy is NULL
		SOCKET result;
		if ((proxy = proxysocketconfig_create_direct()) == nullptr)
			return SOCKET_ERROR;
...
//shutdown(sock, SHUT_RDWR);
	...
	if (status != 0)
		write_log_info(
				proxy, PROXYSOCKET_LOG_ERROR,
				"Failed to close connection (error: %i)", status
		);
	else
		write_log_info(
				proxy, PROXYSOCKET_LOG_INFO,
...
//receive data in dynamically (re)allocated buffer
	if ((buf = (char*)malloc(READ_BUFFER_SIZE)) == nullptr)
		return nullptr;
...
//read data up to and including line break
		if ((n = recv(sock, buf + bufpos, n, 0)) <= 0)
			break;
		bufpos += n;
		if ((buf = (char*)realloc(buf, bufpos + READ_BUFFER_SIZE)) == nullptr)
			return nullptr;
	}
...
//add trailing zero
	if (bufpos >= bufsize)
		if ((buf = (char*)realloc(buf, bufpos + 1)) == nullptr)
			return nullptr;
	buf[bufpos] = 0;
	return buf;
}
...
DLL_EXPORT_PROXYSOCKET char* socket_get_error_message()
{
	...
	if ((errmsg = ::strerrordesc_np(errno)) != nullptr)
	{
...
#endif //PORT_FLOODER_PROXYSOCKS_HPP
